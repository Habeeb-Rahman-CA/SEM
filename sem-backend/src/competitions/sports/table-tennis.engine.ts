import { SportEngine } from './sport-engine.interface';

export class TableTennisEngine implements SportEngine {
  readonly code = 'table_tennis';

  getDefaultConfig(customConfig?: Record<string, any>): Record<string, any> {
    const config = customConfig ?? {};
    if (!config.gamesToWin) config.gamesToWin = 3;
    if (!config.pointsPerGame) config.pointsPerGame = 11;
    return config;
  }

  getInitialLiveData(
    homeTeamId: string,
    awayTeamId: string,
    config?: Record<string, any>,
  ): Record<string, any> {
    return {
      currentGame: 1,
      gamesScore: [{ home: 0, away: 0 }],
      homeGamesWon: 0,
      awayGamesWon: 0,
      events: [],
    };
  }

  calculatePlayerRatings(
    match: any,
    matchPlayers: any[],
    winnerTeamId: string | null,
    loserTeamId: string | null,
  ): any[] {
    const liveData = match.liveData;
    if (!liveData || !Array.isArray(liveData.events)) return [];

    const playingStarters = matchPlayers.filter((mp) => mp.isPlaying);
    if (playingStarters.length === 0) return [];

    const toSave: any[] = [];
    const playerStats = new Map<
      string,
      { aces: number; winners: number; errors: number }
    >();

    for (const mp of playingStarters) {
      playerStats.set(mp.playerId, { aces: 0, winners: 0, errors: 0 });
    }

    for (const ev of liveData.events) {
      const pId =
        ev.playerId ||
        matchPlayers.find((mp) => mp.player?.userId === ev.playerUserId)
          ?.playerId;
      if (!pId) continue;

      const stats = playerStats.get(pId);
      if (!stats) continue;

      if (ev.type === 'ace') stats.aces++;
      else if (ev.type === 'winner') stats.winners++;
      else if (ev.type === 'error') stats.errors++;
    }

    for (const mp of playingStarters) {
      if (mp.rating !== null) continue;

      const stats = playerStats.get(mp.playerId) || {
        aces: 0,
        winners: 0,
        errors: 0,
      };
      let rating = 5.0;

      rating += stats.aces * 0.3;
      rating += stats.winners * 0.2;
      rating -= stats.errors * 0.15;

      if (winnerTeamId && mp.teamId === winnerTeamId) {
        rating += 0.5;
      } else if (loserTeamId && mp.teamId === loserTeamId) {
        rating -= 0.3;
      }

      mp.rating = Math.min(10.0, Math.max(5.0, Math.round(rating * 100) / 100));
      toSave.push(mp);
    }

    return toSave;
  }

  getCompetitionStats(
    completedMatches: any[],
    allMatchPlayers: any[],
    context: {
      userUserIdMap: Map<string, any>;
      userUsernameMap: Map<string, any>;
    },
  ): Record<string, any> {
    const { userUserIdMap } = context;
    const aces = new Map<string, any>();
    const winners = new Map<string, any>();

    const getOrCreateEntry = (
      map: Map<string, any>,
      pUserId: string,
      metricKey: string,
    ) => {
      let entry = map.get(pUserId);
      if (!entry) {
        const info = userUserIdMap.get(pUserId) ?? {
          playerId: pUserId,
          playerName: 'Unknown',
          teamName: 'Unknown',
        };
        entry = { ...info, [metricKey]: 0 };
        map.set(pUserId, entry);
      }
      return entry;
    };

    for (const m of completedMatches) {
      const events = m.liveData?.events;
      if (!Array.isArray(events)) continue;

      for (const ev of events) {
        const pUserId = ev.playerUserId;
        if (!pUserId) continue;

        if (ev.type === 'ace') {
          const entry = getOrCreateEntry(aces, pUserId, 'aces');
          entry.aces++;
        } else if (ev.type === 'winner') {
          const entry = getOrCreateEntry(winners, pUserId, 'winners');
          entry.winners++;
        }
      }
    }

    return {
      topAces: Array.from(aces.values())
        .sort((a, b) => b.aces - a.aces)
        .slice(0, 10),
      topWinners: Array.from(winners.values())
        .sort((a, b) => b.winners - a.winners)
        .slice(0, 10),
    };
  }
}
