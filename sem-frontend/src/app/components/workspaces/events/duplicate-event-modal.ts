...
import { Workspace, WorkspaceEvent } from '../../../services/workspace.service';
        ...
        <div class="px-6 py-5 border-b border-white/5 flex items-center justify-between">
          <div>
            ...
            <p class="text-xs text-slate-400 mt-1">
              Create a new event cycle by duplicating <span class="text-violet-400 font-semibold">{{ targetEvent()?.name }}</span>
            </p>
          </div>
        ...
        <div class="p-6 overflow-y-auto flex-1">
          ...
          <form (submit)="onSubmit(); $event.preventDefault()" class="flex flex-col gap-5">
            ...
            <div class="flex flex-col gap-1.5 text-left">
              <label for="dup-e-name" class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">New Event Name <span class="text-rose-400">*</span></label>
              <input id="dup-e-name" type="text" placeholder="e.g. Annual Sports Meet 2029" autocomplete="off"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="flex flex-col gap-1.5 text-left">
                <label for="dup-e-start" class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Start Date</label>
                <input id="dup-e-start" type="datetime-local" (click)="showDatePicker($event)"
                  ...
                  [ngModel]="startDate()" (ngModelChange)="startDate.set($event)" name="startDate" />
              </div>
              <div class="flex flex-col gap-1.5 text-left">
                <label for="dup-e-end" class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">End Date</label>
                <input id="dup-e-end" type="datetime-local" (click)="showDatePicker($event)"
                  class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none transition-all w-full"
                  [ngModel]="endDate()" (ngModelChange)="endDate.set($event)" name="endDate" />
              </div>
            </div>
            ...
            <div class="flex items-center gap-2 my-2">
              ...
              <span class="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Duplication Preferences</span>
              <div class="h-[1px] bg-white/5 flex-1"></div>
            </div>
            ...
            <div class="flex flex-col gap-3">
              <!-- Competitions -->
              <div (click)="duplicateCompetitions.set(!duplicateCompetitions())"
                ...
                [class]="duplicateCompetitions() ? 'border-violet-500/40 bg-violet-500/5' : 'border-white/5 bg-slate-950/40'">
                ...
                <div class="w-4 h-4 rounded border flex items-center justify-center transition-all"
                  [class]="duplicateCompetitions() ? 'bg-violet-600 border-violet-600 text-white' : 'border-white/20 bg-slate-950'">
                  @if (duplicateCompetitions()) { <i class="fi fi-rr-check text-[8px]"></i> }
                </div>
              </div>

              <!-- Stages -->
              @if (duplicateCompetitions()) {
              <div (click)="duplicateStages.set(!duplicateStages())"
                class="border hover:border-violet-500/30 rounded-xl px-4 py-2.5 flex items-center justify-between gap-3 cursor-pointer transition-all select-none"
                [class]="duplicateStages() ? 'border-violet-500/40 bg-violet-500/5' : 'border-white/5 bg-slate-950/40'">
                <div class="flex flex-col text-left gap-0.5">
                  <span class="text-xs font-bold text-white">Stages & Brackets</span>
                  <span class="text-[10px] text-slate-400">Copy matches' stages (e.g. Group stage, Knockout)</span>
                </div>
                <div class="w-4 h-4 rounded border flex items-center justify-center transition-all"
                  [class]="duplicateStages() ? 'bg-violet-600 border-violet-600 text-white' : 'border-white/20 bg-slate-950'">
                  @if (duplicateStages()) { <i class="fi fi-rr-check text-[8px]"></i> }
                </div>
              </div>
              }
              ...
              <!-- Teams -->
              @if (duplicateCompetitions()) {
              <div (click)="duplicateTeams.set(!duplicateTeams())"
                class="border hover:border-violet-500/30 rounded-xl px-4 py-2.5 flex items-center justify-between gap-3 cursor-pointer transition-all select-none"
                [class]="duplicateTeams() ? 'border-violet-500/40 bg-violet-500/5' : 'border-white/5 bg-slate-950/40'">
                <div class="flex flex-col text-left gap-0.5">
                  <span class="text-xs font-bold text-white">Team Enrollments</span>
                  <span class="text-[10px] text-slate-400">Enroll the same teams in copy-created competitions</span>
                </div>
                <div class="w-4 h-4 rounded border flex items-center justify-center transition-all"
                  [class]="duplicateTeams() ? 'bg-violet-600 border-violet-600 text-white' : 'border-white/20 bg-slate-950'">
                  @if (duplicateTeams()) { <i class="fi fi-rr-check text-[8px]"></i> }
                </div>
              </div>
              }

              <!-- Point Systems -->
              <div (click)="duplicatePointSystems.set(!duplicatePointSystems())"
                class="border hover:border-violet-500/30 rounded-xl px-4 py-2.5 flex items-center justify-between gap-3 cursor-pointer transition-all select-none"
                [class]="duplicatePointSystems() ? 'border-violet-500/40 bg-violet-500/5' : 'border-white/5 bg-slate-950/40'">
                <div class="flex flex-col text-left gap-0.5">
                  <span class="text-xs font-bold text-white">Point Systems & Scoring</span>
                  <span class="text-[10px] text-slate-400">Preserve win/draw/loss point allocations</span>
                </div>
                <div class="w-4 h-4 rounded border flex items-center justify-center transition-all"
                  [class]="duplicatePointSystems() ? 'bg-violet-600 border-violet-600 text-white' : 'border-white/20 bg-slate-950'">
                  @if (duplicatePointSystems()) { <i class="fi fi-rr-check text-[8px]"></i> }
                </div>
              </div>

              <!-- Venues -->
              <div (click)="duplicateVenues.set(!duplicateVenues())"
                class="border hover:border-violet-500/30 rounded-xl px-4 py-2.5 flex items-center justify-between gap-3 cursor-pointer transition-all select-none"
                [class]="duplicateVenues() ? 'border-violet-500/40 bg-violet-500/5' : 'border-white/5 bg-slate-950/40'">
                <div class="flex flex-col text-left gap-0.5">
                  <span class="text-xs font-bold text-white">Venue Assignments</span>
                  <span class="text-[10px] text-slate-400">Copy venue connections to the new event</span>
                </div>
                <div class="w-4 h-4 rounded border flex items-center justify-center transition-all"
                  [class]="duplicateVenues() ? 'bg-violet-600 border-violet-600 text-white' : 'border-white/20 bg-slate-950'">
                  @if (duplicateVenues()) { <i class="fi fi-rr-check text-[8px]"></i> }
                </div>
              </div>

              <!-- General Settings -->
              <div (click)="duplicateSettings.set(!duplicateSettings())"
                class="border hover:border-violet-500/30 rounded-xl px-4 py-2.5 flex items-center justify-between gap-3 cursor-pointer transition-all select-none"
                [class]="duplicateSettings() ? 'border-violet-500/40 bg-violet-500/5' : 'border-white/5 bg-slate-950/40'">
                <div class="flex flex-col text-left gap-0.5">
                  <span class="text-xs font-bold text-white">Public Settings & Announcements</span>
                  <span class="text-[10px] text-slate-400">Copy gallery images, visibility, and announcements</span>
                </div>
                <div class="w-4 h-4 rounded border flex items-center justify-center transition-all"
                  [class]="duplicateSettings() ? 'bg-violet-600 border-violet-600 text-white' : 'border-white/20 bg-slate-950'">
                  @if (duplicateSettings()) { <i class="fi fi-rr-check text-[8px]"></i> }
                </div>
              </div>
            </div>
