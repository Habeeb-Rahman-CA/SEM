...
export class WorkspaceDashboardComponent {
  ...
  attentionItems = computed(() => this.pendingTasks().filter(t => t.severity === 'error' || t.severity === 'warning'));
  ...
  recentActivities = computed(() => {
    ...
    const sorted = [...wsNotifications].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  totalCompletedMatchesCount = computed(() => {
    ...
    for (const comp of this.runningCompetitions()) {
      if (comp.standings) {
        const sumPlayed = comp.standings.reduce((acc: number, row: any) => acc + (row.played || 0), 0);
