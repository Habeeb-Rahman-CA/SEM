...
export class WorkspaceService {
  ...
  bulkImportMembers(
    workspaceId: string,
    payload: { members: { username: string; role?: string }[]; password: string },
  ): Observable<{ success: any[]; failed: any[] }> {
  ...
  uploadImage(
    file: File,
    type: 'workspace' | 'team' | 'user' | 'event' | 'venue',
  ...
  deleteWorkspaceFile(workspaceId: string, fileId: string): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/${workspaceId}/files/${fileId}`, {
      headers: this.headers,
    });
  }
  ...
  globalSearch(
    workspaceId: string,
    query: string,
  ): Observable<{ files: WorkspaceFile[]; teams: Team[]; players: Player[] }> {
