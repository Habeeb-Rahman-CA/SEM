...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-white">
      <div
        class="max-w-md w-full bg-slate-900 border border-white/5 rounded-2xl p-8 text-center shadow-2xl"
      >
        <div class="flex justify-center mb-6">
          <div
            class="w-16 h-16 rounded-2xl bg-violet-600/10 border border-violet-500/20 flex items-center justify-center text-violet-400"
          >
        ...
        @if (isLoading()) {
          ...
          <p class="text-slate-400 text-xs mb-6">
            Please wait while we add you to the workspace members.
          </p>
  ...
  ngOnInit() {
    ...
    this.workspaceService.joinWorkspace(workspaceId).subscribe({
      ...
      error: (err) => {
        ...
        this.error.set(
          err.error?.message ||
            'Could not join the workspace. The link might be invalid or you do not have permission.',
        );
        this.isLoading.set(false);
      },
    });
