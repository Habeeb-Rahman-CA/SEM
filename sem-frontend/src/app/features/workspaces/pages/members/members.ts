...
export class WorkspaceMembersComponent {
  ...
  onUpdateRole(member: WorkspaceMember, event: Event) {
    ...
    this.members.update((prev) =>
      prev.map((m) => (m.id === member.id ? { ...m, role: { ...m.role, slug: newRoleSlug } } : m)),
    );
  ...
  onMembersExcelParsed(json: any[]) {
      ...
      .map((row: any) => {
        ...
        if (!username) {
          ...
          const alreadyExists = this.members().some(
            (m) => m.user.username.toLowerCase() === lowerUser,
          );
          if (alreadyExists) {
            status = 'exist';
    ...
    if (parsedMembers.length === 0) {
      this.memberBulkImportError.set(
        'No valid members found in the spreadsheet. Make sure you have a "Username" column.',
      );
    }
  }
    ...
    this.workspaceService.bulkImportMembers(ws.id, payload).subscribe({
      next: (res) => {
        ...
        membersToImport.forEach((item) => {
          const successItem = res.success.find(
            (s: any) => s.username.toLowerCase() === item.username.toLowerCase(),
          );
          const failedItem = res.failed.find(
            (f: any) => f.username.toLowerCase() === item.username.toLowerCase(),
          );
        ...
        if (failCount === 0) {
          ...
          this.memberBulkImportSuccess.set(
            `Import finished: ${successCount} successful, ${failCount} failed.`,
          );
        }
