...
export class PlayerListComponent {
  ...
  async downloadTemplate() {
    try {
      ...
      const ws: any = {
        ...
        A2: {
          v: '#Required (must exist on system)',
          t: 's',
          s: { font: { color: { rgb: '4B525D' } } },
        },
  ...
  onPlayersExcelParsed(json: any[]) {
    const parsedPlayers = json
      .map((row: any) => {
        const usernameKey =
          Object.keys(row).find((k) => k.toLowerCase() === 'username') || 'Username';
        const teamCodeKey =
          Object.keys(row).find((k) => k.toLowerCase() === 'teamcode') || 'TeamCode';
        const jerseyKey =
          Object.keys(row).find(
            (k) => k.toLowerCase() === 'jerseynumber' || k.toLowerCase() === 'jersey',
          ) || 'JerseyNumber';
        ...
        const team = this.teams().find(
          (t) => t.code && t.code.toUpperCase() === teamCode.toUpperCase(),
        );
        ...
        if (!username) {
          ...
          const alreadyExists = this.players().some(
            (p) => p.user.username.toLowerCase() === username.toLowerCase() && p.teamId === team.id,
          );
          if (alreadyExists) {
            status = 'exist';
    ...
    if (parsedPlayers.length === 0) {
      this.bulkImportError.set(
        'No valid players found in the spreadsheet. Make sure you have a "Username" column.',
      );
    }
  }
  ...
  async onConfirmBulkImport() {
    ...
    for (let i = 0; i < playersToImport.length; i++) {
      ...
      const member = this.members().find(
        (m) => m.user.username.toLowerCase() === item.username.toLowerCase(),
      );
      const team = this.teams().find(
        (t) => t.code && t.code.toUpperCase() === item.teamCode.toUpperCase(),
      );
