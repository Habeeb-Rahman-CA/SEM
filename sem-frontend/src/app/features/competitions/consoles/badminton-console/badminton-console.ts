...
export class BadmintonConsoleComponent implements OnDestroy {
  ...
  onServerChange(serverName: string) {
    ...
    if (match) {
      ...
      const serverTeamPlayers = isHomeServer
        ? homePlayers
        : this.getPlayersForTeam(match.awayTeamId);
      const receiverTeamPlayers = isHomeServer
        ? this.getPlayersForTeam(match.awayTeamId)
        : homePlayers;

      const currentReceiver = this.badmintonReceiver();
      const isReceiverInvalid =
        !currentReceiver || serverTeamPlayers.some((p) => p.user.username === currentReceiver);
  ...
  onRecordBadmintonRally(winnerSide: 'home' | 'away') {
    ...
    live.rallies.push({
      ...
      serviceNumber,
      winner:
        winnerSide === 'home' ? match.homeTeam?.name || 'Home' : match.awayTeam?.name || 'Away',
      winnerSide,
      reason,
      duration,
      scoreAfter: {
        home: isDraft ? setScore.home : setScore.home + (winnerSide === 'home' ? 1 : 0),
        away: isDraft ? setScore.away : setScore.away + (winnerSide === 'away' ? 1 : 0),
      },
      published: !isDraft,
    });
    ...
    if (setWon) {
      ...
      dbStatus = 'live';
      live.matchStatus =
        currentSetNum === 1 ? 'FirstGame' : currentSetNum === 2 ? 'SecondGame' : 'ThirdGame';
    }
  ...
  onUndoBadmintonRally() {
    ...
    if (live.homeSetsWon >= setsToWin || live.awaySetsWon >= setsToWin) {
      ...
      live.matchStatus =
        live.currentSet === 1 ? 'FirstGame' : live.currentSet === 2 ? 'SecondGame' : 'ThirdGame';
    }
      ...
      .subscribe({
        next: (updated) => {
          ...
          this.uiService.success(
            'Badminton statistics validated and published to live scoreboards!',
          );
          if (dbStatus === 'completed') {
            this.matchCompleted.emit();
          }
