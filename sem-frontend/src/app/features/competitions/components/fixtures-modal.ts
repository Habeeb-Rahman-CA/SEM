...
import { Venue } from '../../venues/services/venue.service';
import { CompetitionService } from '../services/competition.service';
import { UiService } from '../../../core/services/ui.service';
import { AvatarComponent } from '../../../shared/components/avatar/avatar';
import { FixtureTemplateService, FixtureTemplate } from '../services/fixture-template.service';
          ...
          <div class="px-6 py-5 border-b border-white/5 flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-slate-400 mt-1">
                Configure competition format, groups, stages and select participating teams.
              </p>
            </div>
            <button
              (click)="isOpen.set(false)"
          ...
          <div class="p-6 overflow-y-auto flex-1 text-left">
            ...
            >
              ...
              <div class="flex flex-col gap-4">
                <div class="flex flex-col gap-1.5 border-b border-white/5 pb-4">
                  <label
                    for="f-fixture-template"
                    class="text-[10px] font-bold text-violet-400 uppercase tracking-wider flex items-center gap-1.5"
                  >
                    <i class="fi fi-rr-calendar-clock text-xs"></i>
                    <span>Apply Scheduling Template</span>
                  </label>
                  <select
                    id="f-fixture-template"
                    ...
                    [ngModel]="selectedFixtureTemplateId()"
                    (ngModelChange)="selectedFixtureTemplateId.set($event)"
                    name="selectedFixtureTemplateId"
                  >
                    <option value="">No template (use manual scheduling defaults)</option>
                    @for (tmpl of fixtureTemplates(); track tmpl.id) {
                      <option [value]="tmpl.id">
                        {{ tmpl.name }} ({{
                          tmpl.venueStrategy === 'single_venue' ? 'Single Venue' : 'Rotated'
                        }}, {{ tmpl.defaultKickoffTime || 'No default time' }})
                      </option>
                    }
                  </select>
                  <p class="text-[10px] text-slate-500">
                    Automatically schedules round-robin/bracket kickoffs, match intervals and
                    venues.
                  </p>
                </div>
                ...
                <div class="flex flex-col gap-1.5">
                  ...
                  <input
                    id="f-stage-name"
                    type="text"
                    placeholder="e.g. Group Stage, Main Draw"
                    class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none w-full"
                    [ngModel]="stageName()"
                    (ngModelChange)="stageName.set($event)"
                    name="stageName"
                    required
                  />
                </div>
                ...
                <div class="flex flex-col gap-1.5">
                  ...
                  <select
                    ...
                    class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none w-full"
                    [ngModel]="stageType()"
                    (ngModelChange)="stageType.set($event)"
                    name="stageType"
                  >
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div class="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        [ngModel]="gamesPerTeam()"
                        (ngModelChange)="gamesPerTeam.set($event)"
                        name="gamesPerTeam"
                      />
                    </div>
                    <div class="flex flex-col gap-1.5">
                      ...
                      <select
                        ...
                        [ngModel]="twoLegged() ? 'true' : 'false'"
                        (ngModelChange)="twoLegged.set($event === 'true')"
                        name="twoLegged"
                      >
                  ...
                  <div class="grid grid-cols-2 gap-4 mt-4">
                    <div class="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none text-center"
                        [ngModel]="restDays()"
                        (ngModelChange)="restDays.set($event)"
                        name="restDays"
                      />
                    </div>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div class="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none text-center"
                        [ngModel]="groupsCount()"
                        (ngModelChange)="groupsCount.set($event)"
                        name="groupsCount"
                      />
                    </div>
                    <div class="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        max="4"
                        class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none text-center"
                        [ngModel]="advancingCount()"
                        (ngModelChange)="advancingCount.set($event)"
                        name="advancingCount"
                      />
                    </div>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div class="flex flex-col gap-1.5">
                      <label
                        for="f-de-reset"
                        class="text-[10px] font-bold text-slate-400 uppercase tracking-wider"
                        >Grand Final Reset</label
                      >
                      <select
                        id="f-de-reset"
                        class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none"
                        [ngModel]="bracketReset() ? 'true' : 'false'"
                        (ngModelChange)="bracketReset.set($event === 'true')"
                        name="deReset"
                      >
                    ...
                    <div class="flex flex-col gap-1.5">
                      ...
                      <select
                        id="f-de-seeded"
                        class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none"
                        [ngModel]="seeded() ? 'true' : 'false'"
                        (ngModelChange)="seeded.set($event === 'true')"
                        name="deSeeded"
                      >
                ...
                @if (stageType() === 'group_knockout') {
                  <div
                    class="flex flex-col gap-4 border border-white/5 bg-slate-950/20 rounded-2xl p-4"
                  >
                    <div class="flex flex-col gap-1.5">
                      <label
                        for="f-gk-subtype"
                      ...
                      <select
                        id="f-gk-subtype"
                        class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none"
                        [ngModel]="groupKnockoutSubtype()"
                        (ngModelChange)="groupKnockoutSubtype.set($event)"
                        name="gkSubtype"
                      >
                        <option value="multiple_groups">
                          Multiple Groups (Groups A, B... to Knockouts)
                        </option>
                        <option value="single_group">
                          Single Group Standings (Top teams to Finals)
                        </option>
                      </select>
                    </div>
                    ...
                    @if (groupKnockoutSubtype() === 'multiple_groups') {
                      <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col gap-1.5">
                          <label
                            for="f-gk-advtype"
                            class="text-[10px] font-bold text-slate-400 uppercase tracking-wider"
                            >Qualified per Group</label
                          >
                          <select
                            id="f-gk-advtype"
                            class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none"
                            [ngModel]="advancingType()"
                            (ngModelChange)="advancingType.set($event)"
                            name="gkAdvType"
                          >
                      ...
                      <div class="flex flex-col gap-1.5">
                        <label
                          for="f-gk-advcount"
                          class="text-[10px] font-bold text-slate-400 uppercase tracking-wider"
                          >Teams Advancing to Knockouts</label
                        >
                        <select
                          id="f-gk-advcount"
                          class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none"
                          [ngModel]="singleGroupAdvancing()"
                          (ngModelChange)="singleGroupAdvancing.set($event)"
                          name="gkAdvCount"
                        >
                ...
                @if (stageType() === 'group' || stageType() === 'group_knockout') {
                  <div
                    class="flex flex-col gap-4 border border-white/5 bg-slate-950/20 rounded-2xl p-4 mt-2 col-span-2"
                  >
                    <h4 class="text-xs font-bold text-violet-400">
                      Custom Qualification & Tie-Breaks
                    </h4>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      />
                      <label for="q-manual" class="text-xs text-slate-300"
                        >Require Manual Approval before advancing</label
                      >
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                      <div class="flex flex-col gap-1.5">
                        ...
                        <input
                          ...
                          max="8"
                          class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none text-center"
                          [ngModel]="runnersUpCount()"
                          (ngModelChange)="runnersUpCount.set($event)"
                          name="runnersUpCount"
                        />
                      </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                      <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider"
                        >Standings Tie-Breakers Order</label
                      >
                      <div class="grid grid-cols-2 gap-2 mt-1">
                        <div class="flex items-center gap-2">
                          <input
                            id="tb-pts"
                            type="checkbox"
                            [checked]="tieBreaks().includes('points')"
                            (change)="toggleTieBreak('points')"
                            class="rounded border-white/10 bg-slate-950 text-violet-600 focus:ring-violet-500"
                          />
                        ...
                        <div class="flex items-center gap-2">
                          <input
                            id="tb-gd"
                            type="checkbox"
                            [checked]="tieBreaks().includes('gd')"
                            (change)="toggleTieBreak('gd')"
                            class="rounded border-white/10 bg-slate-950 text-violet-600 focus:ring-violet-500"
                          />
                        ...
                        <div class="flex items-center gap-2">
                          <input
                            id="tb-gf"
                            type="checkbox"
                            [checked]="tieBreaks().includes('gf')"
                            (change)="toggleTieBreak('gf')"
                            class="rounded border-white/10 bg-slate-950 text-violet-600 focus:ring-violet-500"
                          />
                        ...
                        <div class="flex items-center gap-2">
                          <input
                            id="tb-wins"
                            type="checkbox"
                            [checked]="tieBreaks().includes('wins')"
                            (change)="toggleTieBreak('wins')"
                            class="rounded border-white/10 bg-slate-950 text-violet-600 focus:ring-violet-500"
                          />
                        ...
                        <div class="flex items-center gap-2">
                          <input
                            id="tb-h2h"
                            type="checkbox"
                            [checked]="tieBreaks().includes('h2h')"
                            (change)="toggleTieBreak('h2h')"
                            class="rounded border-white/10 bg-slate-950 text-violet-600 focus:ring-violet-500"
                          />
                        ...
                        <div class="flex items-center gap-2">
                          <input
                            id="tb-custom"
                            type="checkbox"
                            [checked]="tieBreaks().includes('custom')"
                            (change)="toggleTieBreak('custom')"
                            class="rounded border-white/10 bg-slate-950 text-violet-600 focus:ring-violet-500"
                          />
                          <label for="tb-custom" class="text-xs text-slate-300"
                            >Custom Overrides</label
                          >
                        </div>
                      </div>
                    </div>
                    ...
                    @if (tieBreaks().includes('custom')) {
                      <div class="flex flex-col gap-1.5 border-t border-white/5 pt-2">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider"
                          >Custom Override Values</label
                        >
                        <div
                          class="grid grid-cols-2 gap-2 max-h-[150px] overflow-y-auto custom-scrollbar"
                        >
                          @for (teamId of selectedFixtureTeamIds(); track teamId) {
                            <div
                              class="flex items-center justify-between bg-slate-950/60 p-2 rounded-xl border border-white/5"
                            >
                              <span class="text-[11px] text-white truncate max-w-[120px]">{{
                                getTeamName(teamId)
                              }}</span>
                              <input
                                type="number"
                                class="w-12 bg-slate-900 text-white text-xs text-center border border-white/10 rounded px-1"
                                [ngModel]="customOverrides()[teamId] || 0"
                                (ngModelChange)="setCustomOverride(teamId, $event)"
                                name="override-{{ teamId }}"
                              />
                            </div>
                          }
                        </div>
                  ...
                  >
                    <div class="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none text-center"
                        [ngModel]="swissRounds()"
                        (ngModelChange)="swissRounds.set($event)"
                        name="swissRounds"
                      />
                      <p class="text-[10px] text-slate-500">
                        If empty, defaults to Math.ceil(log2(teams)).
                      </p>
                    </div>
                    <div class="flex flex-col gap-1.5">
                      <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider"
                        >Tie-Break Priority</label
                      >
                      <div class="grid grid-cols-2 gap-2 mt-1">
                        <div class="flex items-center gap-2">
                          <input
                            id="tb-buchholz"
                            type="checkbox"
                            [checked]="swissTieBreaks().includes('buchholz')"
                            (change)="toggleSwissTieBreak('buchholz')"
                            class="rounded border-white/10 bg-slate-950 text-violet-600 focus:ring-violet-500"
                          />
                        ...
                        <div class="flex items-center gap-2">
                          <input
                            id="tb-median"
                            type="checkbox"
                            [checked]="swissTieBreaks().includes('median_buchholz')"
                            (change)="toggleSwissTieBreak('median_buchholz')"
                            class="rounded border-white/10 bg-slate-950 text-violet-600 focus:ring-violet-500"
                          />
                          <label for="tb-median" class="text-xs text-slate-300"
                            >Median Buchholz</label
                          >
                        </div>
                        <div class="flex items-center gap-2">
                          <input
                            id="tb-sb"
                            type="checkbox"
                            [checked]="swissTieBreaks().includes('sonneborn_berger')"
                            (change)="toggleSwissTieBreak('sonneborn_berger')"
                            class="rounded border-white/10 bg-slate-950 text-violet-600 focus:ring-violet-500"
                          />
                        ...
                        <div class="flex items-center gap-2">
                          <input
                            id="tb-cumulative"
                            type="checkbox"
                            [checked]="swissTieBreaks().includes('cumulative')"
                            (change)="toggleSwissTieBreak('cumulative')"
                            class="rounded border-white/10 bg-slate-950 text-violet-600 focus:ring-violet-500"
                          />
                          <label for="tb-cumulative" class="text-xs text-slate-300"
                            >Cumulative</label
                          >
                        </div>
                      </div>
                ...
                <div class="grid grid-cols-2 gap-4 border-t border-white/5 pt-4">
                  <div class="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none text-center"
                      [ngModel]="winPoint()"
                      (ngModelChange)="winPoint.set($event)"
                      name="winPoint"
                    />
                  </div>
                  <div class="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none text-center"
                      [ngModel]="drawPoint()"
                      (ngModelChange)="drawPoint.set($event)"
                      name="drawPoint"
                    />
                  </div>
                </div>
                ...
                <div class="flex flex-col gap-1.5">
                  ...
                  <select
                    ...
                    class="bg-slate-950 border border-white/10 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 rounded-xl px-4 py-2.5 text-xs text-white outline-none"
                    [ngModel]="venueId()"
                    (ngModelChange)="venueId.set($event)"
                    name="venueId"
                  >
              ...
              >
                <div class="flex items-center justify-between">
                  <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider"
                    >Select Teams to Enroll</label
                  >
                  <span class="text-[10px] text-violet-400 font-bold"
                    >{{ selectedFixtureTeamIds().length }} selected</span
                  >
                </div>
                ...
                @if (teams().length === 0) {
                  ...
                  >
                    @for (team of teams(); track team.id) {
                      <div
                        (click)="toggleFixtureTeam(team.id)"
                        [class]="
                          selectedFixtureTeamIds().includes(team.id)
                      ...
                      >
                        <div class="flex items-center gap-2">
                          ...
                          />
                          <span class="text-xs font-semibold text-white truncate max-w-[150px]">{{
                            team.name
                          }}</span>
                        </div>
                        <div
              ...
              <div class="md:col-span-2 pt-2 flex gap-2">
                ...
                <button
                  type="submit"
                  [disabled]="isGenerating() || !stageName() || selectedFixtureTeamIds().length < 2"
...
export class FixturesModalComponent {
  ...
  selectedFixtureTemplateId = signal<string>('');

  // Local Form Signals
  stageName = signal('Main Stage');
  stageType = signal<
    'league' | 'group' | 'knockout' | 'group_knockout' | 'double_elimination' | 'swiss'
  >('league');
  ...
  constructor() {
    effect(
      () => {
        ...
        if (open && ws) {
          firstValueFrom(this.fixtureTemplateService.getTemplates(ws.id))
            .then((tmpls) => {
              this.fixtureTemplates.set(tmpls || []);
            })
            .catch((err) => console.error('Failed to load fixture templates', err));
        }
      },
    ...
    effect(
      () => {
        ...
        if (open) {
          this.selectedFixtureTemplateId.set('');
          ...
          if (existingStages.length > 0) {
            ...
            this.swissTieBreaks.set(
              stage.config?.tieBreaks ?? ['buchholz', 'sonneborn_berger', 'cumulative'],
            );
  ...
  async onSubmit() {
    ...
    if (existingStages.length > 0) {
      const confirmed = await this.uiService.confirm({
        ...
        message:
          'This will DELETE any existing matches and regenerate all fixtures randomly. Continue?',
    ...
    try {
      ...
      const refreshedTeams = await firstValueFrom(
        this.competitionService.getCompetitionTeams(ws.id, event.id, comp.id),
      );
      ...
      if (this.stageType() === 'league') {
        ...
        config.groupsCount =
          this.groupKnockoutSubtype() === 'multiple_groups' ? Number(this.groupsCount()) : 1;
        config.advancingType = this.advancingType();
        config.singleGroupAdvancing = Number(this.singleGroupAdvancing());
        config.advancingCount =
          this.groupKnockoutSubtype() === 'multiple_groups'
            ? this.advancingType() === 'winner_and_runner'
              ? 2
              : 1
      ...
      if (existingStages.length > 0) {
        await firstValueFrom(
          this.competitionService.updateStage(
            ws.id,
            event.id,
            comp.id,
            existingStages[0].id,
      ...
      const result = await firstValueFrom(
        this.competitionService.generateFixtures(
          ws.id,
          event.id,
          comp.id,
          this.selectedFixtureTemplateId() || undefined,
        ),
      );

      this.uiService.success(
        `Fixtures generated successfully! Created ${result.matchesCreated} matches.`,
      );
