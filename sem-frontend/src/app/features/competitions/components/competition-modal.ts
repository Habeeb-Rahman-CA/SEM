          ...
          <div class="p-6 overflow-y-auto flex-1 text-left">
            ...
            >
              ...
              <div class="flex flex-col gap-4">
                <div class="flex flex-col gap-1.5">
                  <label
                    ...
                    >Competition Name <span class="text-rose-400">*</span></label
                  >
                  <input
                    id="c-name"
                    type="text"
                    placeholder="e.g. Under-19 Football Tournament"
                    autocomplete="off"
                ...
                <div class="flex flex-col gap-1.5">
                  <label
                    for="c-sport"
                    class="text-[10px] font-bold text-slate-400 uppercase tracking-wider"
                    >Sport <span class="text-rose-400">*</span></label
                  >
                  <select
                    id="c-sport"
                    ...
                    [ngModel]="sportId()"
                    (ngModelChange)="sportId.set($event)"
                    name="cSport"
                    required
                  >
              ...
              <div class="flex flex-col gap-3 md:border-l border-white/5 md:pl-6">
                ...
                <div
                  class="flex flex-col gap-2 max-h-[220px] overflow-y-auto pr-1 custom-scrollbar"
                >
                  @for (row of pointsConfig(); track row.position; let idx = $index) {
                    <div
                      class="flex items-center gap-2 bg-slate-950/40 border border-white/5 rounded-xl p-2.5"
                    >
                      <div class="w-10 text-center font-bold text-xs text-violet-400">
                        #{{ row.position }}
                      </div>
                      <input
                        type="text"
                        [ngModel]="row.label"
                        (ngModelChange)="updatePointsRow(idx, 'label', $event)"
                      ...
                      <input
                        type="number"
                        [ngModel]="row.points"
                        (ngModelChange)="updatePointsRow(idx, 'points', $event)"
                        placeholder="Points"
                      ...
                      <button
                        type="button"
                        (click)="removePointsRow(idx)"
                        class="text-rose-450 hover:text-rose-300 text-xs font-bold transition p-1 cursor-pointer"
                      >
                        ✕
                      </button>
                    </div>
                  } @empty {
                    <p class="text-[11px] text-slate-500 italic py-4 text-center">
                      No points configuration added yet. By default, this competition will not yield
                      overall event leaderboard points.
                    </p>
                  }
                </div>
              </div>
              ...
              <div class="md:col-span-2 pt-2 flex gap-2">
                ...
                <button
                  type="submit"
                  [disabled]="isSaving() || !name() || !sportId()"
...
export class CompetitionModalComponent {
  ...
  addPointsRow() {
    ...
    this.pointsConfig.set([
      ...current,
      {
        ...
        label: defaultLabels[nextPosition] ?? `${nextPosition}th Place`,
        points: 0,
      },
    ]);
  }
  ...
  updatePointsRow(index: number, field: keyof PointsConfigEntry, value: any) {
    this.pointsConfig.update((rows) =>
      rows.map((r, i) =>
        i === index
          ? { ...r, [field]: field === 'points' || field === 'position' ? Number(value) : value }
          : r,
      ),
    );
  }
      ...
      next: (savedComp) => {
        ...
        this.success.set(
          comp
            ...
            : `Competition "${savedComp.name}" created successfully!`,
        );
