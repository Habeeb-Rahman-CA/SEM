    ...
    <div class="flex flex-col gap-6 w-full text-slate-100">
      ...
      <div class="flex flex-col gap-8 overflow-x-auto pb-4 custom-scrollbar">
          ...
          <div class="flex flex-col gap-3 min-w-[800px]">
            ...
            <div class="flex gap-6 items-stretch">
              @for (round of wbRounds(); track round.name) {
                <div
                  class="flex-1 min-w-[240px] flex flex-col justify-around bg-slate-900/30 border border-white/5 rounded-2xl p-4 gap-4"
                >
                  <div
                    class="text-[10px] font-bold text-slate-400 border-b border-white/5 pb-2 uppercase tracking-wide"
                  >
                    {{ round.name }}
                  </div>
                  @for (match of round.matches; track match.id) {
                    ...
                    >
                      @if (match.status === 'live') {
                        <div
                          class="absolute top-0 right-0 w-2 h-2 bg-emerald-500 rounded-full animate-ping m-2"
                        ></div>
                      }
                      ...
                      <div class="flex items-center justify-between text-xs">
                        <div class="flex items-center gap-2 max-w-[170px] truncate">
                          <app-avatar
                            ...
                            customClass="w-5 h-5 rounded-md"
                            textClass="text-[8px] font-bold"
                          />
                          <span
                            [class.text-slate-400]="!match.homeTeam"
                            [class.font-semibold]="match.homeTeam"
                            class="truncate"
                          >
                            {{
                              match.homeTeam?.name ||
                                'Winner of slot ' +
                                  (match.config?.matchSlot !== undefined
                                    ? match.config.matchSlot * 2
                                    : 'TBD')
                            }}
                          </span>
                        </div>
                      ...
                      <div class="flex items-center justify-between text-xs">
                        <div class="flex items-center gap-2 max-w-[170px] truncate">
                          ...
                          />
                          <span
                            [class.text-slate-400]="!match.awayTeam"
                            [class.font-semibold]="match.awayTeam"
                            class="truncate"
                          >
                            {{
                              match.awayTeam?.name ||
                      ...
                      <!-- Footer details -->
                      <div
                        class="border-t border-white/5 pt-2 flex items-center justify-between text-[9px] text-slate-500"
                      >
        ...
        @if (activeTab() === 'all' || activeTab() === 'loser') {
          <div class="flex flex-col gap-3 min-w-[800px]">
            <div
              class="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5 pt-4 border-t border-white/5"
            >
              <i class="fi fi-rr-circle-arrow-down text-rose-400"></i> Loser Bracket (One loss away
              from elimination)
            </div>
            <div class="flex gap-6 items-stretch">
        ...
        @if (activeTab() === 'all' || activeTab() === 'finals') {
          <div class="flex flex-col gap-3 min-w-[400px]">
            <div
              class="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5 pt-4 border-t border-t-white/5"
            >
            ...
            <div class="flex gap-6 items-stretch">
              @for (match of gfMatches(); track match.id) {
                <div
                  class="flex-1 min-w-[240px] flex flex-col bg-slate-900/30 border border-white/5 rounded-2xl p-4 gap-4 justify-center"
                >
                  <div
                    class="text-[10px] font-bold text-slate-400 border-b border-white/5 pb-2 uppercase tracking-wide flex justify-between"
                  >
                    <span>{{ match.config?.round }}</span>
                    @if (match.config?.bracket === 'grand_final_reset') {
                      <span
                        class="text-rose-400 text-[8px] bg-rose-500/10 px-1 rounded border border-rose-500/20"
                        >Reset Match</span
                      >
                    }
                  </div>
                    ...
                    >
                      <span class="capitalize flex items-center gap-1">
                        @if (match.status === 'live') {
                          ...
                          match.config?.bracket === 'grand_final_reset' &&
...
export class DoubleEliminationBracketComponent {
  ...
  gfMatches = computed(() => {
    return this.matches()
      .filter(
        (m) => m.config?.bracket === 'grand_final' || m.config?.bracket === 'grand_final_reset',
      )
      .sort((a, b) => {
