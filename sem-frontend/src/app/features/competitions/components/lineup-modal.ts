          ...
          <div class="px-6 py-4 border-b border-white/5 flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-slate-400 mt-0.5">
                Select the starting lineup and designated goalkeeper for both teams.
              </p>
            </div>
            <button
              (click)="isOpen.set(false)"
          ...
          <div class="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
            ...
            <div class="flex flex-col gap-4">
              <div class="flex items-center gap-2 border-b border-white/5 pb-2">
                <app-avatar
                  [name]="selectedMatch()?.homeTeam?.name || 'HM'"
                  [logoUrl]="selectedMatch()?.homeTeam?.logoUrl"
                  customClass="w-7 h-7 rounded-lg"
                  textClass="text-[10px] font-black text-white"
                />
                <h4 class="text-sm font-bold text-white">
                  {{ selectedMatch()?.homeTeam?.name }} Starting Lineup
                </h4>
              </div>
              ...
              <div class="flex flex-col gap-2 max-h-[380px] overflow-y-auto pr-1 custom-scrollbar">
                @for (item of getHomePlayersInForm(); track item.playerId) {
                  <div
                    class="flex items-center justify-between gap-3 p-2 border border-white/5 rounded-xl bg-slate-950/20"
                  >
                    <div class="flex items-center gap-2.5 min-w-0">
                      <button
                        (click)="togglePlayerInLineup(item.playerId)"
                      ...
                      <div class="min-w-0">
                        ...
                        <p class="text-[10px] text-slate-500">
                          Jersey #{{ item.player.jerseyNumber || 'N/A' }}
                        </p>
                      </div>
                    </div>
                    ...
                    @if (item.isPlaying) {
                      <button
                        (click)="setGoalkeeper(selectedMatch()!.homeTeamId!, item.playerId)"
                  ...
                  <p class="text-xs text-slate-500 italic text-center py-4">
                    No players registered under this team.
                  </p>
                }
              </div>
            </div>
            ...
            <div class="flex flex-col gap-4">
              <div class="flex items-center gap-2 border-b border-white/5 pb-2">
                <app-avatar
                  [name]="selectedMatch()?.awayTeam?.name || 'AW'"
                  [logoUrl]="selectedMatch()?.awayTeam?.logoUrl"
                  customClass="w-7 h-7 rounded-lg"
                  textClass="text-[10px] font-black text-white"
                />
                <h4 class="text-sm font-bold text-white">
                  {{ selectedMatch()?.awayTeam?.name }} Starting Lineup
                </h4>
              </div>
              ...
              <div class="flex flex-col gap-2 max-h-[380px] overflow-y-auto pr-1 custom-scrollbar">
                @for (item of getAwayPlayersInForm(); track item.playerId) {
                  ...
                  >
                    ...
                    @if (item.isPlaying) {
                      <button
                        (click)="setGoalkeeper(selectedMatch()!.awayTeamId!, item.playerId)"
                        class="px-2.5 py-1 text-[9px] font-bold rounded-lg border transition-all cursor-pointer"
                        [class]="
                          item.isGoalkeeper
                            ? 'bg-amber-500/10 border-amber-500/30 text-amber-400'
...
export class LineupModalComponent {
  ...
  saveLineup() {
    ...
    this.competitionService
      .saveMatchLineup(ws.id, event.id, comp.id, stage.id, match.id, payload)
      .subscribe({
        next: (updatedLineup) => {
